...
gcloud compute ssh jenkins-server \
  ...
  --project=core-it-infra-prod \
  --tunnel-through-iap \
  --command='nslookup jenkins.np.dreamcompany.intranet'
```
...
Name:   jenkins.np.dreamcompany.intranet
Address: 10.10.10.50
```
...
gcloud compute ssh jenkins-server \
  ...
  --project=core-it-infra-prod \
  --tunnel-through-iap \
  --command='dig jenkins.np.dreamcompany.intranet'
```
...
```powershell
# Test DNS
Resolve-DnsName jenkins.np.dreamcompany.intranet

# Test connectivity
Test-NetConnection -ComputerName 10.10.10.50 -Port 443

# Test HTTPS (ignore certificate warning)
Invoke-WebRequest -Uri https://jenkins.np.dreamcompany.intranet -UseBasicParsing
```

5. **Open Browser**: Navigate to `https://jenkins.np.dreamcompany.intranet`
...
# Should show: CN=jenkins.np.dreamcompany.intranet
```
...
**Symptoms**:
- `jenkins.np.dreamcompany.intranet` does not resolve
...
```bash
# Check A record
gcloud dns record-sets describe jenkins.np.dreamcompany.intranet. \
  --zone=jenkins-private-zone \
  --type=A \
  --project=core-it-infra-prod
