...
gcloud compute ssh jenkins-server --zone=us-central1-a \
  --project=core-it-infra-prod --tunnel-through-iap \
  --command='curl -k https://jenkins.np.dreamcompany.intranet/'
```
- [ ] Returns Jenkins HTML
...
gcloud compute ssh jenkins-server --zone=us-central1-a \
  --project=core-it-infra-prod --tunnel-through-iap \
  --command='curl -k -I https://jenkins.np.dreamcompany.intranet'
...
4. DNS returns: jenkins.np.dreamcompany.intranet = 10.10.10.50
