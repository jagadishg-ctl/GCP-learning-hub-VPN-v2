...
2. Navigate to: `https://jenkins.np.dreamcompany.intranet`
3. You'll see a certificate warning (because CA is not trusted)
4. Click "Advanced" → "Proceed to jenkins.np.dreamcompany.intranet"
...
3. **Test again**:
   - Open browser and navigate to `https://jenkins.np.dreamcompany.intranet`
...
Windows Test Server (10.10.x.x)
     ↓ DNS Query
Cloud DNS → jenkins.np.dreamcompany.intranet = 10.10.10.50
     ↓ HTTPS Request
