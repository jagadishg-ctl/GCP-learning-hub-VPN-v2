...
resource "google_compute_instance" "dns_forwarder" {
  ...
  metadata_startup_script = <<-EOF
    ...
    # Create dreamcompany domain forwarding config
    cat > /etc/dnsmasq.d/dreamcompany.conf <<DNSMASQ_CONF
server=/dreamcompany.intranet/169.254.169.254
server=8.8.8.8
server=8.8.4.4
DNSMASQ_CONF
...
OVERRIDE_CONF
    ...
    echo "Forwarding *.dreamcompany.intranet to 169.254.169.254"
